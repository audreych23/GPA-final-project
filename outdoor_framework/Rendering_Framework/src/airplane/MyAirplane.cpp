...
MyAirplane::MyAirplane(INANOA::MyCameraManager* cameraManager) {
  ...
  this->dso->setModelMat(glm::translate(glm::vec3(40.0f, 40.0f, 40.0f)));
  this->dso->setPixelFunctionId(1);
}
...
void MyAirplane::render() {
  ...
  glBindTexture(GL_TEXTURE_2D, 0);
  glm::mat4 mat = this->cameraManager->airplaneModelMatrix();
  this->dso->setModelMat(mat);
  this->dso->update();
}

void MyAirplane::update(const glm::mat4& viewMat, const glm::vec3& viewPos) {
