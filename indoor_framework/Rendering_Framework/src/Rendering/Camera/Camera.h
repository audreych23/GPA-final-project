...
namespace INANOA {
	...
	public:
		explicit Camera();
		explicit Camera(const glm::vec3& viewOrg, const glm::vec3& lookCenter, 
			const glm::vec3& upVector, const float distance, const float fovDeg, const float fNear, const float fFar);
		virtual ~Camera();
	...
	public:
		void reset(const glm::vec3& viewOrg, const glm::vec3& lookCenter, 
			const glm::vec3& upVector, const float distance);
		...
		// forward is used on player and god camera
		...
		// for god camera
		void rotateUpDownLeftRight(float x_offset, float y_offset, bool constraint_pitch);
