	...
	const GLuint fsFunctionHandle, const int pixelProcessFuncBinding) 
{
	...
	this->m_house = new Model("assets\\Medieval_Building_LowPoly\\medieval_building_lowpoly_2.obj", 
		"assets\\cityLots_sub_0.ppd2", "assets\\Medieval_Building_LowPoly\\Medieval_Building_LowPoly_V2_Albedo_small.png", 3.0f);
	this->m_house2 = new Model("assets\\Medieval_Building_LowPoly\\medieval_building_lowpoly_1.obj", 
		"assets\\cityLots_sub_1.ppd2", "assets\\Medieval_Building_LowPoly\\Medieval_Building_LowPoly_V1_Albedo_small.png", 4.0f);
	// pass in 3 model max num vertex, pass in 3 model maxnum index
	std::cout << m_modelBush01->getNumSamples() << " " << m_modelBush05->getNumSamples() << " " << m_modelGrass->getNumSamples() << " " << m_house->getNumSamples() << " " << m_house2->getNumSamples();
	m_totalInstance = m_modelBush01->getNumSamples() + m_modelBush05->getNumSamples() + 
		m_modelGrass->getNumSamples() + m_house->getNumSamples() + m_house2->getNumSamples();
...
void MyBushesAndBuildings::setupDrawCommand() {
	...
	int maxNumVertex = modelMesh1[0].vertices.size() + 
		...
		+ modelMesh4[0].vertices.size() + modelMesh5[0].vertices.size();
	int maxNumIndices = modelMesh1[0].indices.size() + 
		modelMesh2[0].indices.size() + modelMesh3[0].indices.size()
		+ modelMesh4[0].indices.size() + modelMesh5[0].indices.size();
	...
	offset += m_modelGrass->getNumSamples();
	for (int i = 0; i < m_house->getNumSamples(); ++i) {
		const float* instPos = m_house->getInstancePositions(i);
		rawInstData[offset + i].position = glm::vec4(instPos[0], instPos[1], instPos[2], 3.0f);
		const float* instRad = m_house->getInstanceRadians(i);
		glm::quat q = glm::quat(glm::vec3(instRad[0], instRad[1], instRad[2]));
		rawInstData[offset + i].rotMatrix = glm::toMat4(q);
	}

	offset += m_house->getNumSamples();
	for (int i = 0; i < m_house2->getNumSamples(); ++i) {
		const float* instPos = m_house2->getInstancePositions(i);
		rawInstData[offset + i].position = glm::vec4(instPos[0], instPos[1], instPos[2], 4.0f);
		const float* instRad = m_house2->getInstanceRadians(i);
		glm::quat q = glm::quat(glm::vec3(instRad[0], instRad[1], instRad[2]));
		rawInstData[offset + i].rotMatrix = glm::toMat4(q);
	}
	...
	offset += 9 * modelMesh1[0].vertices.size();
	...
	offset += 9 * modelMesh2[0].vertices.size();
	...
	offset += 9 * modelMesh3[0].vertices.size();
	for (unsigned int i = 0; i < modelMesh4[0].vertices.size(); ++i) {
		data[offset + i * 9 + 0] = modelMesh4[0].vertices[i].position.x;
		data[offset + i * 9 + 1] = modelMesh4[0].vertices[i].position.y;
		data[offset + i * 9 + 2] = modelMesh4[0].vertices[i].position.z;
		data[offset + i * 9 + 3] = modelMesh4[0].vertices[i].normal.x;
		data[offset + i * 9 + 4] = modelMesh4[0].vertices[i].normal.y;
		data[offset + i * 9 + 5] = modelMesh4[0].vertices[i].normal.z;
		data[offset + i * 9 + 6] = modelMesh4[0].vertices[i].tex_coords.x;
		data[offset + i * 9 + 7] = modelMesh4[0].vertices[i].tex_coords.y;
		data[offset + i * 9 + 8] = modelMesh4[0].vertices[i].tex_coords.z;
	}

	offset += 9 * modelMesh4[0].vertices.size();
	for (unsigned int i = 0; i < modelMesh5[0].vertices.size(); ++i) {
		data[offset + i * 9 + 0] = modelMesh5[0].vertices[i].position.x;
		data[offset + i * 9 + 1] = modelMesh5[0].vertices[i].position.y;
		data[offset + i * 9 + 2] = modelMesh5[0].vertices[i].position.z;
		data[offset + i * 9 + 3] = modelMesh5[0].vertices[i].normal.x;
		data[offset + i * 9 + 4] = modelMesh5[0].vertices[i].normal.y;
		data[offset + i * 9 + 5] = modelMesh5[0].vertices[i].normal.z;
		data[offset + i * 9 + 6] = modelMesh5[0].vertices[i].tex_coords.x;
		data[offset + i * 9 + 7] = modelMesh5[0].vertices[i].tex_coords.y;
		data[offset + i * 9 + 8] = modelMesh5[0].vertices[i].tex_coords.z;
	}
	...
	offset += modelMesh3[0].indices.size();
	for (unsigned int i = 0; i < modelMesh4[0].indices.size(); ++i) {
		indexData[offset + i] = modelMesh4[0].indices[i];
	}

	offset += modelMesh4[0].indices.size();
	for (unsigned int i = 0; i < modelMesh5[0].indices.size(); ++i) {
		indexData[offset + i] = modelMesh5[0].indices[i];
	}
	...
	// first house model
	drawCommands[3].count = modelMesh4[0].indices.size();
	drawCommands[3].instanceCount = m_house->getNumSamples();
	drawCommands[3].firstIndex = modelMesh1[0].indices.size() + modelMesh2[0].indices.size() + modelMesh3[0].indices.size();
	drawCommands[3].baseVertex = modelMesh1[0].vertices.size() + modelMesh2[0].vertices.size() + modelMesh3[0].vertices.size();
	drawCommands[3].baseInstance = m_modelBush01->getNumSamples() + m_modelBush05->getNumSamples() + m_modelGrass->getNumSamples();

	//// second house model
	drawCommands[4].count = modelMesh5[0].indices.size();
	drawCommands[4].instanceCount = m_house2->getNumSamples();
	drawCommands[4].firstIndex = modelMesh1[0].indices.size() + modelMesh2[0].indices.size() + 
		modelMesh3[0].indices.size() + modelMesh4[0].indices.size();
	drawCommands[4].baseVertex = modelMesh1[0].vertices.size() + modelMesh2[0].vertices.size() + 
		modelMesh3[0].vertices.size() + modelMesh4[0].vertices.size();
	drawCommands[4].baseInstance = m_modelBush01->getNumSamples() + m_modelBush05->getNumSamples() + m_modelGrass->getNumSamples() + m_house->getNumSamples();
	...
	glBufferStorage(GL_SHADER_STORAGE_BUFFER, sizeof(DrawElementsIndirectCommand) * 5,
